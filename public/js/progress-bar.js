...
$(document).ready(function() {
    ...
    $('.progressbar-header').eq(0).before('<div class="progressbar"></div>');
    ...
    $('.progressbar-header').each(function() {
        var code = '<i data-label="' + $(this).text() + '"></i>';
        labels.append(code);
    });
    var points = labels.find('i');
    points.css('width', 100 / $('.progressbar-header').length + '%');
    ...
    function setPosition() {
        if (container.is(':visible')) {
            ...
            if (currentPosition < $('.progressbar-header').eq(0).offset().top) {
            ...
            // if within document
                ...
                var startSection = $('.progressbar-header').eq(section);
                var endSection = $('.progressbar-header').eq(section + 1);
    ...
    points.click(function() {
        ...
        var targetY = $('.progressbar-header').eq(sectionIndex).offset().top - (triggerPoint * .92);
        $('html, body').animate({
            scrollTop: targetY
        }, 600, 'easeInOutCubic');
    });
