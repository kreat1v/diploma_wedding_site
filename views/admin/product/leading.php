...
<div class="container">
    <div class="category">
        ...
        <?php if (isset($data['product'])): ?>
            ...
            <div class="bt text">
                <a class="sm-buttons" href="<?=$router->buildUri('product.leading', ['new'])?>">
            ...
            <div class="table text">
    			<table>
    				...
    				<tr>
    					<td><?=$value['id_leading']?></td>
    					<td><?=$value['title']?></td>
                        <td><?=$value['active'] == 1 ? __('admin_product.activity2') : '-'?></td>
                        <td><?=$value['stock'] ? __('admin_product.stock2') : '-'?></td>
                        <?php if ($value['active'] == 1): ?>
    					<td>
                            <a class="sm-buttons" href="<?=$router->buildUri('default.leading.view', [$value['id_leading']])?>">
                        ...
                        <?php endif; ?>
    					<td>
                            <a class="sm-buttons" href="<?=$router->buildUri('admin.product.leading', ['edit', $value['id_leading']])?>">
            ...
            <nav>
                <ul class="pagination text">
                    <li class="<?=$data['pagination']['back'] ? '' : 'disabled'?>">
                        <a href="<?=$data['pagination']['back'] ? $router->buildUri('admin.product.leading', [1]) : ''?>">
                            <span>&laquo;</span>
                        </a>
                    </li>

                    <li class="<?=$data['pagination']['back'] ? '' : 'disabled'?>">
                        <a href="<?=$data['pagination']['back'] ? $router->buildUri('admin.product.leading', [$data['pagination']['back']]) : ''?>"><?=__('pagination.previous')?></a>
                    </li>

                    <?php foreach ($data['pagination']['middle'] as $key => $value): ?>
                    <li class="<?=$data['page'] == $key ? 'active' : ''?>">
                        <a href="<?=$router->buildUri('admin.product.leading', [$key])?>"><?=$key?></a>
                    </li>
                    <?php endforeach; ?>

                    <li class="<?=$data['pagination']['forward'] ? '' : 'disabled'?>">
                        <a href="<?=$data['pagination']['forward'] ? $router->buildUri('admin.product.leading', [$data['pagination']['forward']]) : ''?>"><?=__('pagination.next')?></a>
                    </li>

                    <li class="<?=$data['pagination']['forward'] ? '' : 'disabled'?>">
                        <a href="<?=$router->buildUri('admin.product.leading', [$data['pagination']['last']])?>">
                            <span>&raquo;</span>
                        </a>
                    </li>
                </ul>
                    ...
                    <div id="galery">
                        ...
                        <?php if(isset($data['edit']['galery']) && $data['edit']['galery'] != false): ?>
                            ...
                            <div class="file-upload"
                                ...
                                <img class="product-image" style="display: block;" src="<?=Config::get('leadingImg') . $data['edit']['main']['id'] . DS . $img?>" />
