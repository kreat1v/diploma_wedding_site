...
<div class="container">
    <div class="category">
        ...
        <?php if (isset($data['product'])): ?>
            ...
            <div class="bt text">
                <a class="sm-buttons" href="<?=$router->buildUri('product.hotel', ['new'])?>">
            ...
            <div class="table text">
    			<table>
    				...
    				<tr>
    					...
    					<td>
                            <a class="sm-buttons" href="<?=$router->buildUri('default.hotel.view', [$value['id_hotel']])?>">
                        ...
                        <?php endif; ?>
    					<td>
                            <a class="sm-buttons" href="<?=$router->buildUri('admin.product.hotel', ['edit', $value['id_hotel']])?>">
            ...
            <nav>
                <ul class="pagination text">
                    <li class="<?=$data['pagination']['back'] ? '' : 'disabled'?>">
                        <a href="<?=$data['pagination']['back'] ? $router->buildUri('admin.product.hotel', [1]) : ''?>">
                            <span>&laquo;</span>
                        </a>
                    </li>

                    <li class="<?=$data['pagination']['back'] ? '' : 'disabled'?>">
                        <a href="<?=$data['pagination']['back'] ? $router->buildUri('admin.product.hotel', [$data['pagination']['back']]) : ''?>"><?=__('pagination.previous')?></a>
                    </li>

                    <?php foreach ($data['pagination']['middle'] as $key => $value): ?>
                    <li class="<?=$data['page'] == $key ? 'active' : ''?>">
                        <a href="<?=$router->buildUri('admin.product.hotel', [$key])?>"><?=$key?></a>
                    </li>
                    <?php endforeach; ?>

                    <li class="<?=$data['pagination']['forward'] ? '' : 'disabled'?>">
                        <a href="<?=$data['pagination']['forward'] ? $router->buildUri('admin.product.hotel', [$data['pagination']['forward']]) : ''?>"><?=__('pagination.next')?></a>
                    </li>

                    <li class="<?=$data['pagination']['forward'] ? '' : 'disabled'?>">
                        <a href="<?=$router->buildUri('admin.product.hotel', [$data['pagination']['last']])?>">
                            <span>&raquo;</span>
                        </a>
                    </li>
                </ul>
        ...
        <?php else: ?>
            ...
            <form class="form text" id="product-form" method="post" enctype="multipart/form-data">
                ...
                <fieldset>
                    ...
                    <label>
                        <span><?=__('admin_product.stars')?></span>
                        <label id="stars">
                            <input type="radio" name="stars" value="1" class="option-input radio" <?=$edit && $data['edit']['main']['stars'] == 1 ? 'checked' : ''?> />
                            <span>
                                <i class="fas fa-star"></i>
                            </span>
                            <input type="radio" name="stars" value="2" class="option-input radio" <?=$edit && $data['edit']['main']['stars'] == 2 ? 'checked' : ''?> />
                            <span>
                                <?php for ($i = 1; $i <= 2; $i++): ?>
                                <i class="fas fa-star"></i>
                                <?php endfor; ?>
                            </span>
                            <input type="radio" name="stars" value="3" class="option-input radio" <?=$edit && $data['edit']['main']['stars'] == 3 ? 'checked' : ''?> />
                            <span>
                                <?php for ($i = 1; $i <= 3; $i++): ?>
                                <i class="fas fa-star"></i>
                                <?php endfor; ?>
                            </span>
                            <input type="radio" name="stars" value="4" class="option-input radio" <?=$edit && $data['edit']['main']['stars'] == 4 ? 'checked' : ''?> />
                            <span>
                                <?php for ($i = 1; $i <= 4; $i++): ?>
                                <i class="fas fa-star"></i>
                                <?php endfor; ?>
                            </span>
                            <input type="radio" name="stars" value="5" class="option-input radio" <?=$edit && $data['edit']['main']['stars'] == 5 ? 'checked' : ''?> />
                            <span>
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                <i class="fas fa-star"></i>
                                <?php endfor; ?>
                    ...
                    <div id="galery">
                        ...
                        <?php if(isset($data['edit']['galery']) && $data['edit']['galery'] != false): ?>
                            ...
                            <div class="file-upload"
                                ...
                                <img class="product-image" style="display: block;" src="<?=Config::get('hotelImg') . $data['edit']['main']['id'] . DS . $img?>" />
