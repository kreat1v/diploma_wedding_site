        ...
        <div class="product view-product">
            ...
            <div class="gradient-border">
                ...
                <div class="goods">
                    ...
                    <?php
                    $image = Config::get('hotelImgRoot') . $data['product']['id_hotel'] . DS . $data['galery'][0];
                    ...
                    <div class="blur <?=$blurClass?>">
                        <img src="<?=Config::get('hotelImg') . $data['product']['id_hotel'] . DS . $data['galery'][0]?>"/>
                    </div>
                    ...
                    <div class="bt">
                        ...
                        <form method="post">
                            <input class="id_products" type="hidden" name="id_products" value="<?=$data['product']['id_hotel']?>">
                            ...
                            <?php if(!in_array($data['product']['id_hotel'] . $data['category'], $data['favorites'])): ?>
                            <form method="post">
                                <input class="id_products" type="hidden" name="id_products" value="<?=$data['product']['id_hotel']?>">
                                <input class="category" type="hidden" name="category" value="<?=lcfirst($router->getController(true))?>">
                                <input class="id_users" type="hidden" name="id_users" value="<?=\App\Core\App::getSession()->get('id')?>">
                            ...
                            <?php endif; ?>
                        <?php endif; ?>
                        <a href="<?=$router->buildUri('hotel.reviews', [$data['product']['id_hotel']])?>" class="sm-buttons text">
                    ...
                    <div class="galery">
                        <div class="panels">
                            ...
                            <a href="<?=\App\Core\Config::get('hotelImg') . $data['product']['id_hotel'] . DS . $img?>" class="panel">
                                <div class="panel__content" style="background-image: url('<?=\App\Core\Config::get('hotelImgWeb') . $data['product']['id_hotel'] . '/' . $img?>');"></div>
                            </a>
                            <?php endforeach; ?>
                        </div>
                    </div>
