            ...
            <?php if (!empty($data['product'])):
                ...
                <div class="gradient-border">
                    ...
                    <div class="goods">
                        ...
                        <?php
                        $image = Config::get('leadingImgRoot') . $value['id_leading'] . DS . $value['galery'][0];
                        ...
                        <div class="blur <?=$blurClass?>">
                            <img src="<?=Config::get('leadingImg') . $value['id_leading'] . DS . $value['galery'][0]?>"/>
                        </div>
                        ...
                        <div class="bt">
                            <?php if(Session::get('id') && Session::get('role') == 'user'): ?>
                                <?php if(empty(Session::get('cart')['leading'])): ?>
                                <form method="post">
                                    <input class="id_products" type="hidden" name="id_products" value="<?=$value['id_leading']?>">
                                ...
                                <?php endif; ?>
                                <?php if(!in_array($value['id_leading'] . $data['category'], $data['favorites'])): ?>
                                <form method="post">
                                    <input class="id_products" type="hidden" name="id_products" value="<?=$value['id_leading']?>">
                                    <input class="category" type="hidden" name="category" value="<?=lcfirst($router->getController(true))?>">
                                ...
                                <?php endif; ?>
                            <?php endif; ?>
                            <a href="<?=$router->buildUri('leading.reviews', [$value['id_leading']])?>" class="sm-buttons text">
                        ...
                        <div class="galery">
                            <div class="panels">
                                ...
                                <a href="<?=\App\Core\Config::get('leadingImg') . $value['id_leading'] . DS . $img?>" class="panel">
                                    <div class="panel__content" style="background-image: url('<?=\App\Core\Config::get('leadingImgWeb') . $value['id_leading'] . '/' . $img?>');"></div>
                                </a>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        ...
                        <div class="mob-gallery">
                            <?php foreach ($value['galery'] as $img): ?>
                            <a href="<?=\App\Core\Config::get('leadingImg') . $value['id_leading'] . DS . $img?>" style="background-image: url('<?=\App\Core\Config::get('leadingImgWeb') . $value['id_leading'] . '/' . $img?>');"></a>
                            <?php endforeach; ?>
                        </div>
                    </div>
                ...
                <nav>
                    <ul class="pagination text">
                        <li class="<?=$data['pagination']['back'] ? '' : 'disabled'?>">
                            <a href="<?=$data['pagination']['back'] ? $router->buildUri('leading.index', [1]).$filter : ''?>">
                                <span>&laquo;</span>
                            </a>
                        </li>

                        <li class="<?=$data['pagination']['back'] ? '' : 'disabled'?>">
                            <a href="<?=$data['pagination']['back'] ? $router->buildUri('leading.index', [$data['pagination']['back']]).$filter : ''?>"><?=__('pagination.previous')?></a>
                        </li>

                        <?php foreach ($data['pagination']['middle'] as $key => $value): ?>
                        <li class="<?=$data['page'] == $key ? 'active' : ''?>">
                            <a href="<?=$router->buildUri('leading.index', [$key]).$filter?>"><?=$key?></a>
                        </li>
                        <?php endforeach; ?>

                        <li class="<?=$data['pagination']['forward'] ? '' : 'disabled'?>">
                            <a href="<?=$data['pagination']['forward'] ? $router->buildUri('leading.index', [$data['pagination']['forward']]).$filter : ''?>"><?=__('pagination.next')?></a>
                        </li>

                        <li class="<?=$data['pagination']['forward'] ? '' : 'disabled'?>">
                            <a href="<?=$router->buildUri('leading.index', [$data['pagination']['last']]).$filter?>">
                                <span>&raquo;</span>
                            </a>
                        </li>
                    </ul>
        ...
        <div class="filter text">
            ...
            <form method="get" id="filter">
                ...
                <div class="bt">
                    ...
                    <div>
                        <a class="sm-buttons text" href="<?=$router->buildUri('.leading')?>"><?=__('filter.reset')?></a>
                    </div>
                </div>
            </form>
        </div>
