        ...
        <div class="product view-product">
            ...
            <div class="gradient-border">
                ...
                <div class="goods">
                    ...
                    <?php
                    $image = Config::get('autoImgRoot') . $data['product']['id_auto'] . DS . $data['galery'][0];
                    ...
                    <div class="blur <?=$blurClass?>">
                        <img src="<?=Config::get('autoImg') . $data['product']['id_auto'] . DS . $data['galery'][0]?>"/>
                    </div>
                    ...
                    <div class="bt">
                        <?php if(Session::get('id') && Session::get('role') == 'user'): ?>
                            <?php if(empty(Session::get('cart')['auto'])): ?>
                            <form method="post">
                                <input class="id_products" type="hidden" name="id_products" value="<?=$data['product']['id_auto']?>">
                            ...
                            <?php endif; ?>
                            <?php if(!in_array($data['product']['id_auto'] . $data['category'], $data['favorites'])): ?>
                            <form method="post">
                                <input class="id_products" type="hidden" name="id_products" value="<?=$data['product']['id_auto']?>">
                                <input class="category" type="hidden" name="category" value="<?=lcfirst($router->getController(true))?>">
                            ...
                            <?php endif; ?>
                        <?php endif; ?>
                        <a href="<?=$router->buildUri('auto.reviews', [$data['product']['id_auto']])?>" class="sm-buttons text">
                ...
                <div class="mob-gallery">
                    <?php foreach ($data['galery'] as $img): ?>
                    <a href="<?=\App\Core\Config::get('autoImg') . $data['product']['id_auto'] . DS . $img?>" style="background-image: url('<?=\App\Core\Config::get('autoImgWeb') . $data['product']['id_auto'] . '/' . $img?>');"></a>
                    <?php endforeach; ?>
                </div>
