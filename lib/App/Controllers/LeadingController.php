...
class LeadingController extends Base
{
	private $userModel;
		...
		if ($category['active'] != 0) {
			...
			if (!empty($_GET)) {
				if (array_key_exists('price', $_GET)) {
					...
					$maxVal = $this->leadingMainModel->getMaxPrice();
			...
			$productsCount = count($this->leadingMainModel->languageList($get));
			...
			if (!$pagination['page404']) {
				$this->data['filter']['maxPrice'] = $this->leadingMainModel->getMaxPrice()['max'];
				$this->data['title'] = $category['full_title'];
				$this->data['text'] = $category['second_text'];
				$this->data['page'] = $page;
				$this->data['product'] = $this->leadingMainModel->languageList($get, [Config::get('pagLimit'), $offset]);
				...
				foreach ($this->data['product'] as $key => $value) {
					...
					if (file_exists(Config::get('leadingImgRoot') . $value['id_leading'])) {
						$this->data['product'][$key]['galery'] = array_values(array_diff(scandir(Config::get('leadingImgRoot') . $value['id_leading']), ['.', '..']));
					} else {
	...
	public function reviewsAction()
	{
		...
		if ($category['active'] != 0) {
			...
			if (!empty($params)) {
				...
				$product = $this->leadingMainModel->languageList(['id' => $id]);

				// Получение изображения товара, а так же класса стиля для этого изображения.
				if (file_exists(Config::get('leadingImgRoot') . $id)) {
					$paths = array_values(array_diff(scandir(Config::get('leadingImgRoot') . $id), ['.', '..']));
					$avatar = Config::get('leadingImg') . $id . DS . $paths[0];

					$imageRoot = Config::get('leadingImgRoot') . $id . DS . $paths[0];
				...
				App::getRouter()->redirect(App::getRouter()->buildUri('.leading'));
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				try {
					...
					if (isset($_POST['button']) && $_POST['button'] == 'send') {
						...
						App::getRouter()->redirect(App::getRouter()->buildUri('leading.reviews', [$id]));
					}
	...
	public function getReviewsAction()
	{
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$reviews = $this->leadingReviewsModel->reviews(['id_product' => $id_product,'active' => 1], [$limit, $start]);
	...
	public function viewAction()
	{
		...
		if ($category['active'] != 0) {
			...
			if (!empty($params)) {
				...
				if (file_exists(Config::get('leadingImgRoot') . $id)) {
					$galery = array_values(array_diff(scandir(Config::get('leadingImgRoot') . $id), ['.', '..']));
				} else {
					$galery = false;
				}
