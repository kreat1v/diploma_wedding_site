...
class FilmingController extends Base
{
	private $userModel;
		...
		if ($category['active'] != 0) {
			...
			if (!empty($_GET)) {
				if (array_key_exists('price', $_GET)) {
					...
					$maxVal = $this->filmingMainModel->getMaxPrice();
			...
			$productsCount = count($this->filmingMainModel->languageList($get));
			...
			if (!$pagination['page404']) {
				$this->data['filter']['maxPrice'] = $this->filmingMainModel->getMaxPrice()['max'];
				$this->data['title'] = $category['full_title'];
				$this->data['text'] = $category['second_text'];
				$this->data['page'] = $page;
				$this->data['product'] = $this->filmingMainModel->languageList($get, [Config::get('pagLimit'), $offset]);
				...
				foreach ($this->data['product'] as $key => $value) {
					...
					if (file_exists(Config::get('filmingImgRoot') . $value['id_filming'])) {
						$this->data['product'][$key]['galery'] = array_values(array_diff(scandir(Config::get('filmingImgRoot') . $value['id_filming']), ['.', '..']));
					} else {
	...
	public function reviewsAction()
	{
		...
		if ($category['active'] != 0) {
			...
			if (!empty($params)) {
				...
				if (file_exists(Config::get('filmingImgRoot') . $id)) {
					$paths = array_values(array_diff(scandir(Config::get('filmingImgRoot') . $id), ['.', '..']));
					$avatar = Config::get('filmingImg') . $id . DS . $paths[0];

					$imageRoot = Config::get('filmingImgRoot') . $id . DS . $paths[0];
				...
				App::getRouter()->redirect(App::getRouter()->buildUri('.filming'));
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				try {
					...
					if (isset($_POST['button']) && $_POST['button'] == 'send') {
						...
						App::getRouter()->redirect(App::getRouter()->buildUri('filming.reviews', [$id]));
					}
	...
	public function getReviewsAction()
	{
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$reviews = $this->filmingReviewsModel->reviews(['id_product' => $id_product,'active' => 1], [$limit, $start]);
	...
	public function viewAction()
	{
		...
		if ($category['active'] != 0) {
			...
			if (!empty($params)) {
				...
				if (file_exists(Config::get('filmingImgRoot') . $id)) {
					$galery = array_values(array_diff(scandir(Config::get('filmingImgRoot') . $id), ['.', '..']));
				} else {
					$galery = false;
				}
